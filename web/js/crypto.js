/**
 * SilentChat Web Client - Crypto Operations
...
// Generate a single RSA key pair that we'll use for both signing AND encryption
// This matches final.py which uses one key pair for both purposes
export async function generateKeyPair() {
    // We generate as RSA-OAEP first (for encryption), then export/reimport for signing
    ...
    // Export the keys to PKCS8/SPKI format
    const privateKeyPkcs8 = await crypto.subtle.exportKey("pkcs8", keyPair.privateKey);
    const publicKeySpki = await crypto.subtle.exportKey("spki", keyPair.publicKey);

    // Re-import the same key material for signing operations
    ...
    return {
        // For encryption/decryption (key exchange)
        encryptPrivateKey: keyPair.privateKey,
        encryptPublicKey: keyPair.publicKey,
        // For signing/verification (same key material, different algorithm)
        signPrivateKey,
        signPublicKey,
        // Raw key material for storage
        privateKeyPkcs8,
        publicKeySpki
    };
}
...
export async function exportPublicKeyPem(publicKeySpki) {
    // Accept either ArrayBuffer or CryptoKey
