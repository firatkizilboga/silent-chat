...
void Database::initDb() {
    ...
    // Messages table with msgId for server ID tracking
    ...
    // Config table for storing lastMessageId and other settings
    ...
    // Try to add msg_id column if it doesn't exist (migration)
    sqlite3_exec(db_, "ALTER TABLE messages ADD COLUMN msg_id INTEGER DEFAULT 0", nullptr, nullptr, nullptr);
}

bool Database::addMessage(const std::string& sigId, const std::string& peer,
                          const std::string& sender, const std::string& content, 
                          double timestamp, int64_t msgId) {
    std::lock_guard<std::mutex> lock(mutex_);
...
std::vector<Message> Database::getMessages(const std::string& peer) {
    std::lock_guard<std::mutex> lock(mutex_);
...
std::vector<std::string> Database::getPeers() {
    std::lock_guard<std::mutex> lock(mutex_);
...
void Database::setLastMessageId(int64_t id) {
    std::lock_guard<std::mutex> lock(mutex_);
