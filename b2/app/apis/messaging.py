...
async def post_message(message: Message, sender_alias: str = Depends(get_current_user)):
    ...
    if message.type == "FILE":
        ...
        await db.add_file(file_id, message_id, sender_alias, message.recipientAlias, encrypted_content)
    else:
        message_id = await db.add_message(full_message)
...
async def websocket_endpoint(websocket: WebSocket, token: str = Query(...)):
    ...
    """
    try:
        # Authenticate the user manually since Depends doesn't work well with WebSockets
    ...
    try:
        while True:
            ...
            # We don't necessarily expect input from the client here for this simple push model,
            # but we need to await something to keep the loop running and detect disconnects.
            data = await websocket.receive_text()
            # If the client sends "ping", we can respond "pong" or just ignore.
            if data == "ping":
...
async def get_messages(
    ...
    timeout_seconds: int = Query(default=25, ge=0, le=60, description="Long polling timeout in seconds")
):
    """
...
async def get_user_public_key(alias: str, current_user: str = Depends(get_current_user)):
    """
